	...
	var ISSelectReplacement = {
		...
		on_click: function(element, id)
		{
			...
			else
			{
				var replacement = element.parentNode.parentNode.nextSibling;
				var option = replacement.childNodes[id[0]];
                if (option == undefined) option = replacement.childNodes[0].childNodes[id[0]-1];
			}

			option.selected = checkbox.checked;
            var checkedTotal = -1;
            for (var i = 0; i < element.parentNode.childNodes.length; i++) {
                var currentElement = element.parentNode.childNodes[i];
                if (currentElement.childNodes[0].checked) checkedTotal++;
            }
            $(element).parents('div')[0].selectedIndex = checkedTotal;
