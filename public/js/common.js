...
$(document).ready(function()
{
    ...
    if(typeof(in_app) != "undefined") {
        var iL = new Image();
        iL.src = "index.php?action=SettingsFooterImage&rnd="+Math.floor(Math.random()*500);
        if(document.location.href.toLowerCase().indexOf('settings') == -1) {
            iL.onerror = function() { document.location.href = "index.php?action=viewSettings&bk=1"; }
        }
...
function display_error(id,message,timeout){
    ...
    if(timeout > 0) {
        ...
        window.setTimeout(function() { $('#'+id).hide('slow'); }, 2000);
    }
}
...
function ChangePaging(object, todo, pagenumber) {
    ...
    document.location = 'index.php?action=' + todo + '&page=' + pagenumber + '&perpage='+ pagingamount;
}
        ...
        $(this).click(function(e) {
            ...
            if ($(this).hasClass('FormFieldsMenuButton')) {
                ...
                if(pageOffsetLeft + menuWidth > $(window).width()) {
                    ...
                    $(menu).css('right', offsetLeft+2+ "px");
                }
            }
...
function ShowLoadingIndicator() {
    ...
    $('#AjaxLoading').css('top', '10px');
    $('#AjaxLoading').css('left', "50%");
    $('#AjaxLoading').css('margin-left', "-50px");
    $('#AjaxLoading').show();
}
...
function openProductSelect(type, select, idlist, single, closeFocus) {
    ...
    windowLocation = 'index.php?action=popupProductSelect';
...
$(document).ready(function() {
    ...
    // TODO: Deixar sÃ³ com on quando terminar a atualizaÃ§Ã£o
    if (typeof $('.CheckboxTogglesOtherElements').on === 'function') {
        $('.CheckboxTogglesOtherElements').on('change', _onChangeCheckboxToggleOtherElements).change();
    } else {
        $('.CheckboxTogglesOtherElements').live('change', _onChangeCheckboxToggleOtherElements).change();
    }


    function _onChangeCheckboxToggleOtherElements(event) {
        if (!this.id) return;
...
var Common = {
    ExportGoogleBase: function()
    {
        $.iModal({
            type: 'ajax',
            url: 'index.php?action=exportFroogleIntro',
            width: 400,
    ...
    ExportNewsletterSubscribers: function()
    {
        $.iModal({
            type: 'ajax',
            url: 'index.php?action=exportSubscribersIntro',
            width: 400,
            onBeforeClose: function() {
    ...
    DisplayGoogleSitemapInfo: function()
    {
        $.iModal({
            type: 'ajax',
            url: 'index.php?action=showGoogleSitemapInfo',
            width: 400
        });
    },
...
$(function(){
    // TODO: Deixar sÃ³ com on quando terminar a atualizaÃ§Ã£o
    if (typeof $('.HelpLink').on === 'function') {
...
$(function(){
    ...
    // TODO: Deixar sÃ³ com on quando terminar a atualizaÃ§Ã£o
    if (typeof $('.showByValue').on === 'function') {
        $('.showByValue').on('change', showByValueOnChange);
    } else {
